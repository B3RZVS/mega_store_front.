...
export default function CheckboxList({ refresh }: { refresh: boolean }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const response = await fetch('http://localhost:8080/products/marcas');
    ...
    <List className={Style.list}>
      ...
      {(data ?? [1,2,3]).map ((marca, idx) => {
        ...
        return (
          ...
          > 
              <ListItemText id={labelId} primary={`${marca.nombre}`} className={Style.item}/>
          </ListItem>
        );
      })}
    </List>
